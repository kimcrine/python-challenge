...
#Export text file
pypoll_output = os.path.join(".", "Analysis", "analysis.txt")
with open(pypoll_output, "w") as txt:
    txt.write(f"Election Results\n")
    txt.write(f"-------------------------\n")
    txt.write(f"Total Votes: {total_votes}\n")
    txt.write(f"-------------------------\n")
    txt.write(f"Khan: {khan_percent:.3%} ({khan_total})\n")
    txt.write(f"Correy: {correy_percent:.3%} ({correy_total})\n")
    txt.write(f"Li: {li_percent:.3%} ({li_total})\n")
    txt.write(f"O'Tooley: {o_tooley_percent:.3%} ({o_tooley_total})\n")
    txt.write(f"-------------------------\n")
